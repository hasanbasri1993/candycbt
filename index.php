<?php
	...
	if ($pg == '') {
		...
		$mapelQ = mysql_query("SELECT * FROM ujian WHERE status='1' ORDER BY tgl_ujian ");
		
		while ($mapelx = mysql_fetch_array($mapelQ)) {
			
			if (date('Y-m-d', strtotime($mapelx['tgl_ujian']))) {
				$datakelas = unserialize($mapelx['kelas']);
				
				if (in_array($kelasx, $datakelas) or in_array('semua', $datakelas)) { //cek kelas
		...
		echo "
														...
														<div class='box-footer navbar-fixed-bottom'>
															...
															<tr><td>
																...
																<div class='col-md-4 '>
																	<i class='fa fa-spin fa-spinner' id='spin-next' style='display:none;'></i><button id='move-next' class='btn  btn-primary' onclick=loadsoal($id_mapel,$id_siswa,$no_next,1)><span class='hidden-xs'>SELANJUTNYA</span> <i class='fa fa-chevron-right'></i></button>
																</div></td>
																</tr>
															</table>
														</div>
