	...
	if ($ac == '') {
		...
		while ($mapel = mysql_fetch_array($mapelQ)) {
			...
			foreach ($dataArray as $key => $value) {
				$kelasL = mysql_fetch_array(mysql_query("SELECT * FROM kelas WHERE id_kelas='$value'"));
				echo "<small class='label label-success'>$kelasL[nama] </small>&nbsp;";
			}
			echo "
														</td>
														<td>
			...
			foreach ($dataArray as $key => $value) {
				$kelasL = mysql_fetch_array(mysql_query("SELECT * FROM kelas WHERE id_kelas='$value'"));
				echo "<option value=$value>$kelasL[nama]</option>";
			}
